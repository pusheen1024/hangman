...
def get_word(word):
    ...
    return res
while True:
    wrd = get_word(word_list).upper()
    word_completion = "_." * len(wrd)  # поле для ответа
    ...
    while tries > 0:
        ...
        print("Введите одну букву в верхнем регистре, которая, по вашему мнению, содержится в слове, или же всё слово целиком (тоже в верхнем регистре), если вы его угадали и так.")
        a = input().upper()
        if a.isalpha() == False:
            print("А может, введёте букву или слово?")
            continue
        elif a in guessed_letters or (a in guessed_words and len(a) == len(wrd)):
            print("Вы уже предлагали это слово или букву.")
            continue
        else:
            if a in wrd and len(a) == 1:
                for i in range(len(wrd)):
                    if wrd[i] == a.upper():
                        wrd_completion = word_completion.split(".")
                        wrd_completion[i] = a
                        word_completion = ".".join(wrd_completion)
                guessed_letters += a
            elif a in wrd and len(a) == len(wrd):
                print("Вы угадали слово! Поздравляем!")
                break
            elif a not in wrd and len(a) == len(wrd):
                print("Вы не угадали слово.")
                guessed_words += a
                tries -= 1
            else:
                print("Вы не угадали букву или слово.")
                guessed_letters += a
                tries -= 1
            print(tr(tries))
        if "_" not in word_completion:
            print("Слово полностью угадано! Поздравляем.")
            break
        if "_" in word_completion and tries == 0:
            print("Увы, Вы проиграли. Загаданное слово -", wrd)
